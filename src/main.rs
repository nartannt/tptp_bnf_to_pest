...
fn concatable(token_rule: Rule) -> bool {
    match token_rule {
        ...
        | Rule::class_pattern
        | Rule::class_bracketed_expansion
        | Rule::regex_bracketed_expansion
        | Rule::class_expansion
        | Rule::regex_expansion => {
            return true;
        }
        _ => {
            return false;
...
fn inner_regex_to_str(pairs: Pairs<Rule>) -> String {
    ...
    while let Some(pair) = pairs.next() {
        ...
        match pair_rule {
            Rule::regex_chars_inner => {
                res.push(
                    "\"".to_string() + &escape_special_chars(pair.as_str().to_string()) + "\"",
                );
            }
            Rule::not => {
                res.push(
                    "(!\"".to_string() + &escape_special_chars(pairs.as_str().to_string()) + "\")",
                );
                return res.join(" ");
            }
            Rule::special_chars => {
            ...
            Rule::chars_range => {
                ...
                if pairs
                    .peek()
                    .is_some_and(|r| r.as_rule() == Rule::chars_range)
                {
                    res.push("|".to_string());
                }
            }
    ...
    while let Some(pair) = pairs.next() {
        ...
        match pair_rule {
            ...
            Rule::class_chars => {
                ...
                if class_chars_inner
                    .peek()
                    .is_some_and(|r| r.as_rule() == Rule::not)
                {
                    res.push(inner_regex_to_str(class_chars_inner));
                } else if class_chars_inner
                    .peek()
                    .is_some_and(|r| r.as_rule() == Rule::chars_range)
                {
                    res.push(inner_regex_to_str(class_chars_inner));
                    ...
                    while class_chars_inner.peek().is_some() {
                        let next_str = "\"".to_string()
                            + &escape_special_chars(
                                class_chars_inner.next().unwrap().as_str().to_string(),
                            )
                            + "\"";
                        res.push(next_str);
